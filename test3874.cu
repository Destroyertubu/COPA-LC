...
// ====================================================================
// Step 1: New Kernel to Count Partitions
// ====================================================================
__global__ void countPartitionsPerBlock(
    ...
    const float* __restrict__ variances,
    int num_variance_blocks,
    int* __restrict__ d_partition_counts, // Output: array of counts per block
    const float* __restrict__ variance_thresholds) 
{
    // Pre-computed thresholds remain the same
    float thresh_low = variance_thresholds[0];
    float thresh_med = variance_thresholds[1];
    float thresh_high = variance_thresholds[2];
    
    // Grid-stride loop remains the same
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; 
         i < num_variance_blocks; 
         i += blockDim.x * gridDim.x) 
    {
        float var = variances[i];
        int block_start = i * base_size * 8;
        int block_end = min(block_start + base_size * 8, data_size);
        
        // Partition size decision logic remains the same
        int partition_size;
        if (var < thresh_low) {
            partition_size = base_size * 4;
        } else if (var < thresh_med) {
        ...
        // **MODIFICATION**: Instead of atomicAdd, count locally
        int count = 0;
        for (int j = block_start; j < block_end; j += partition_size) {
            if (j < data_size) {
                count++;
        ...
        // Write the local count to the temporary array
        d_partition_counts[i] = count;
    }
}

// ====================================================================
// Step 2: New Kernel to Write Partitions using Offsets
// ====================================================================
__global__ void writePartitionsFromOffsets(
    int data_size,
    int base_size,
    const float* __restrict__ variances,
    int num_variance_blocks,
    int* __restrict__ partition_starts, // Final output
    int* __restrict__ partition_ends,   // Final output
    const int* __restrict__ d_partition_offsets, // Input: Calculated offsets
    const float* __restrict__ variance_thresholds)
{
    float thresh_low = variance_thresholds[0];
    float thresh_med = variance_thresholds[1];
         ...
         i += blockDim.x * gridDim.x)
    {
        ...
        // **MODIFICATION**: Use the pre-calculated offset to write results
        int current_offset = d_partition_offsets[i];
        
        for (int j = block_start; j < block_end; j += partition_size) {
            if (j < data_size) {
                // Write partition data directly to the correct location
                partition_starts[current_offset] = j;
                partition_ends[current_offset] = min(j + partition_size, data_size);
                current_offset++; // Increment local offset
            }
        }
    }
}
